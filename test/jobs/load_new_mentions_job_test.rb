...
class LoadNewMentionsJobTest < ActiveJob::TestCase
  setup do
    stub_x
    stub_li
  end

  test "creates mentions" do
    assert_difference -> { Mention.count }, 6 do
      LoadNewMentionsJob.perform_now(organizations(:respondo))
    end
  end
  ...
  def stub_x
    x_mention = mentions(:x)

    stub_request(:get, "https://api.twitter.com/2/users/me").to_return(
      status: 200, headers: { "Content-Type" => "application/json; charset=utf-8" },
      body: file_fixture("x_users_me.json")
    )
    stub_request(:get, "https://api.twitter.com/2/users/2244994945/mentions?expansions=author_id,referenced_tweets.id&since_id=#{x_mention.external_uid}&tweet.fields=created_at").to_return(
      status: 200, headers: { "Content-Type" => "application/json; charset=utf-8" },
