        ...
        context 'when the user is signed out' do
          context 'when the account does not exist' do
            it 'creates a new account' do
              expect { post_user }.to change(UserAccount, :count).from(0).to(1)
            end

            it 'creates a new user' do
              expect { post_user }.to change(User, :count).from(0).to(1)
            end
          ...
          context 'when the account exists' do
            ...
            it 'does not create a new account' do
              expect { post_user }.not_to change(UserAccount, :count).from(1)
            end

            it 'does not create a new user' do
              expect { post_user }.not_to change(User, :count).from(1)
            end
        ...
        context 'when user is signed in' do
          ...
          context 'when account does not exist' do
            context 'when user does not have account for provider' do
              it 'creates a new account' do
                expect { post_user }.to change(UserAccount, :count).from(1).to(2)
              end
              ...
              it 'associates the account with the current user' do
                expect { post_user }.to change { user.reload.accounts.find_by(provider:) }
            ...
            context 'when user already has account for same provider' do
              ...
              it 'does not create a new account' do
                expect { post_user }.not_to change(UserAccount, :count).from(2)
              end
            end
          end
          ...
          context 'when account exists' do
            context 'when account belongs to other user' do
              ...
              it 'does not create a new user' do
                expect { post_user }.not_to change(User, :count).from(2)
              end

              it 'associates the account with the current user' do
                expect { post_user }.to change { account.reload.user }.from(account.user).to(user)
              end
            end
        ...
        context 'when user is signed in' do
          ...
          context 'when brand exists' do
            ...
            context 'when account does not exist' do
              ...
              it 'creates a new account' do
                expect { post_brand }.to change(BrandAccount, :count).from(1).to(2)
              end

              it 'does not create a new brand' do
                expect { post_brand }.not_to change(Brand, :count).from(1)
              end

              it 'associates the account with the current brand' do
                expect { post_brand }.to change { brand.reload.accounts.count }.from(1).to(2)
              end
            ...
            context 'when account exists' do
              context 'when account belongs to other brand' do
                ...
                it 'does not create a new account' do
                  expect { post_brand }.not_to change(BrandAccount, :count).from(1)
                end

                it 'does not create a new user' do
                  expect { post_brand }.not_to change(Brand, :count).from(2)
                end

                it 'associates the account with the current brand' do
                  expect { post_brand }.to change { account.reload.brand }.from(account.brand).to(brand)
                end
          ...
          context 'when brand does not exist' do
            context 'when the account does not exist' do
              it 'creates a new account' do
                expect { post_brand }.to change(BrandAccount, :count).from(0).to(1)
              end

              it 'creates a new user' do
                expect { post_brand }.to change(Brand, :count).from(0).to(1)
              end

              it 'associates the signed in user with the brand' do
                expect { post_brand }.to change { user.reload.brand }.from(nil).to(an_instance_of(Brand))
              end
